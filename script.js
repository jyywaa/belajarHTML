// Initial Products Default Data (Lengkap HP & Laptop)
...
// Safety Check: Bersihkan cache jika terjadi bentrokan struktur data lama
...
function renderProducts() {
    ...
    state.products.forEach(p => {
        ...
        const videoHtml = p.video ? `
            <video class="product-video" controls muted poster="${p.image}" onerror="this.style.display='none'">
...
function openDetail(id) {
    ...
    const videoHtml = p.video ? `<video src="${p.video}" controls style="width:100%; max-height:180px; border-radius:8px; margin-bottom:10px; background:#000;" onerror="this.style.display='none'"></video>` : '';
