// Database IndexedDB untuk penanganan upload file besar (Gambar + Video)
const DB_NAME = "GadgetStoreDB";
...
// SUBMIT ADMIN (Mendukung Upload File Video & Gambar dari HP Tanpa Crash)
async function handleAdminSubmit(e) {
    ...
    const vidFile = document.getElementById('admin-video-file').files[0];

    if (!imgFile) {
        showToast("⚠️ Silakan upload file gambar!");
        return;
    }

    showToast("⌛ Menyimpan produk & mengolah file...");

    // Mengubah Gambar ke Blob
    const imageBlob = new Blob([await imgFile.arrayBuffer()], { type: imgFile.type });
    let videoBlob = null;
    if (vidFile) {
        videoBlob = new Blob([await vidFile.arrayBuffer()], { type: vidFile.type });
    }
    ...
    showToast("✅ Produk & File Video Berhasil Disimpan!");
}
...
// Convert File Blob ke URL yang bisa ditampilkan HTML
...
async function loadProductsFromDB() {
    let dbProds = await dbGetAllProducts();
    if (dbProds.length === 0) {
        for (let p of defaultProducts) {
            await dbAddProduct(p);
        }
        dbProds = await dbGetAllProducts();
    }
    state.products = dbProds.reverse();
    renderProducts();
}
...
function renderProducts() {
    ...
    state.products.forEach(p => {
        ...
        const vidUrl = getMediaUrl(p.video);

        const videoHtml = vidUrl ? `
            <video class="product-video" controls muted poster="${imgUrl}">
                <source src="${vidUrl}">
        ...
        if (p.bestseller) {
            bestsellerContainer.innerHTML += `
                <div class="scroll-item">
                    <img src="${imgUrl}" style="width:100%; height:90px; object-fit:cover; border-radius:6px;">
...
function openDetail(id) {
    ...
    const videoHtml = vidUrl ? `
        <video controls style="width:100%; max-height:200px; border-radius:8px; margin-bottom:10px; background:#000; object-fit:cover;">
            <source src="${vidUrl}">
        </video>` : '';
