// Initial Products Default Data (4 Produk Gadget & Pemanggilan Video GitHub)
const defaultProducts = [
    ...
    {
        id: 104,
        name: "Tablet Pro Ultra 12.9",
        price: 14000000,
        desc: "Layar Liquid Retina, Chip Kencang, Mendukung Stylus & Keyboard.",
...
// Upload File Admin (Produk Tambahan Kamu Tidak Dihapus)
...
function renderProducts() {
    ...
    state.products.forEach(p => {
        ...
        const videoHtml = p.video ? `
            <video class="product-video" controls playsinline preload="metadata" poster="${p.image}">
...
function openDetail(id) {
    ...
    const videoHtml = p.video ? `
        <video controls playsinline style="width:100%; max-height:200px; border-radius:8px; margin-bottom:10px; background:#000; object-fit:cover;">
            <source src="${p.video}" type="video/mp4">
        </video>` : '';
